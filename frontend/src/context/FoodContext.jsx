...
export const FoodProvider = ({ children }) => {
    ...
    const updatePoll = useCallback(async (id, updatedPoll) => {
        try {
            const updated = await pollsAPI.update(id, updatedPoll);
            setPolls(polls.map(p => (p._id || p.id) === id ? updated : p));
        } catch (err) {
            console.error('Error updating poll:', err);
            // Fallback: optimistic update
            setPolls(polls.map(p => (p._id || p.id) === id ? { ...p, ...updatedPoll } : p));
        }
    }, [polls]);
    ...
    const deletePoll = useCallback(async (id) => {
        try {
            ...
            setPolls(polls.filter(p => (p._id || p.id) !== id));
        } catch (err) {
    ...
    const toggleWishlist = useCallback(async (foodId) => {
        try {
            const res = await usersAPI.toggleWishlist(foodId);
            // Update wishlist from response or fetch fresh
            if (res && res.wishlist) {
                // Fetch full food details for the wishlist
                const fullWishlist = await usersAPI.getWishlist();
            ...
            // Fallback: optimistic update on error
            setWishlist(prev => {
                const exists = prev.some(f => (f._id || f.id) === foodId);
                if (exists) return prev.filter(f => (f._id || f.id) !== foodId);
                const food = foods.find(f => (f._id || f.id) === foodId);
    ...
    return (
        <FoodContext.Provider value={{
            ...
            polls, voteForFood: voteForPoll, promoteToMenu, updatePoll, deletePoll,
